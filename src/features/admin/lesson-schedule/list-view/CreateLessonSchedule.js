...
const CreateLessonScheduleModal = ({ isOpen, onClose, onSuccess, schedule }) => {
  ...
  const checkConflict = async (values) => {
    try {
      const response = await checkScheduleConflict(0, values);
  ...
  const handleValuesChange = async (_, allValues) => {
    const { groupId, roomId, startTime, endTime, day } = allValues;
    ...
    }else if(groupId !== undefined && roomId !== undefined && Number.isInteger(startTime) && Number.isInteger(endTime) && Number.isInteger(day)) {
      await checkConflict(allValues);
    }
    else {
      setHasConflict(false);
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      const response = await createLessonSchedule(values);
      const { success, message: responseMessage } = response.data;
      if (success) {
        message.success("Lesson schedule created successfully");
        form.resetFields();
        onSuccess();
      } else {
        message.error(responseMessage || "Failed to create lesson schedule");
      }
    } catch (error) {
      message.error(error.response?.data?.message || "Error creating lesson schedule");
    }
  };

  const handleCancel = useCallback(() => {
    message.info("Lesson schedule creation canceled");
    form.resetFields();
    setHasConflict(false);
    setConflictMessage("");
    onClose();
  ...
  return (
    <Modal
      title="Create New Lesson Schedule"
      open={isOpen}
...
export default CreateLessonScheduleModal;